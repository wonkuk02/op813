...
def long_control_state_trans(CP, active, long_control_state, v_ego, v_target_future,
  ...
  if radarState is not None and radarState.leadOne is not None and radarState.leadOne.status:
    stopping_condition = stopping_condition and (radarState.leadOne.vLead < CP.vEgoStopping or radarState.leadOne.dRel < 6.)

  starting_condition = v_target_future > CP.vEgoStarting and not cruise_standstill

  # neokii
  if radarState is not None and radarState.leadOne is not None and radarState.leadOne.status:
    starting_condition = starting_condition and (radarState.leadOne.vLead > CP.vEgoStarting or radarState.leadOne.dRel > 1.0)
    ...
    # Intention is to stop, switch to a different brake control until we stop
      ...
      if not CS.standstill or output_accel > CP.stopAccel:
        output_accel -= CP.stoppingDecelRate * DT_CTRL * \
                        interp(output_accel, [CP.stopAccel, CP.stopAccel / 2., 0.8], [1., 7.5, 15.])
    ...
    # Intention is to move again, release brake fast before handing control to PID
      ...
      if output_accel < CP.startAccel:
        output_accel += CP.startingAccelRate * DT_CTRL * \
                        interp(output_accel, [CP.startAccel, CP.startAccel / 2., 1.2], [46., 23., 1.])
      self.reset(CS.vEgo)
